...
<div class="col-lg-4">
	...
	if($rowclient['cartedepoint'] !=''){
		$query = "SELECT COALESCE(SUM(points),0) as pointTotal FROM POINTS WHERE num_avantages = '".str_replace(' ', '',$rowclient['cartedepoint'])."'";
					...
					<tbody>
						...
						while($rowgratuit = $resultgratuit->fetch_assoc()){
							if($rowgratuit["id_groupe_fidele"] < 0){
								?>
								<tr>
									<td>Fidélité AnimoEtc 9+1<br /><?php echo formatDateUTF8($rowgratuit['date_insert']);?></td>
									<td class="centre"><a class="ajaxModal" href="javascript:;" data-modal-url="ajax/modals/viewFacture.php?id_facture=<?php echo $rowgratuit['id_facture']; ?>&ID_MAGASIN=<?php echo $rowgratuit['ID_MAGASIN'];?>"><?php echo $rowgratuit['id_facture']; ?></a></td>
								</tr>
								<?php
							}else{
								?>
								<tr>
									<td><?php echo $rowgratuit['nom'];?><br/><a href="?p=rap_fidelite_details&id_achat_fidele_gratuit=<?php echo $rowgratuit['id_achat_fidele_gratuit'];?>&id_groupe_fidele=<?php echo $rowgratuit['id_groupe_fidele'];?>"><?php echo formatDateUTF8($rowgratuit['date_insert']);?></td>
									<td class="centre"><a class="ajaxModal" href="javascript:;" data-modal-url="ajax/modals/viewFacture.php?id_facture=<?php echo $rowgratuit['id_facture']; ?>&ID_MAGASIN=<?php echo $rowgratuit['ID_MAGASIN'];?>"><?php echo $rowgratuit['id_facture']; ?></a></td>
								</tr>
							<?php
						}
