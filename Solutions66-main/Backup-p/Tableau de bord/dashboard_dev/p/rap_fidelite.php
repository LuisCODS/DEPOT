...
$querygratuit = "SELECT achat_fidele_gratuit.*, groupe.nom 
                ...
                GROUP BY achat_fidele_gratuit.id_achat_fidele_gratuit DESC";
...
if ( $_GET["getFile"] == "1" and $arrayData )
{
	...
	$listSoustitre = [];
	$listSoustitre[] = [L("en_date_du","o") , date("Y-m-d") ];
	...
	$listEnteteColonne = [
                    		[ 
                        	    ["text"=>L('ProgrammeFidelite',"o"),"width"=>80,"align"=>"L"],
                                            				...
                                            				<option value="">Tous les fournisseurs</option>
                                        						<div>
                                            						...
                                            						$queryFour = "SELECT fournisseur.id_fournisseur,fournisseur.nom 
                                                                                   ...
                                                                                   where groupe.inactif is null 
                                                                                         and groupe.type = 'fidelite' 
                                                                                    	 and groupe_fidele.inactif is null 
                                                                                    	 and fournisseur.inactif is null 
													...
													<tr>
														<th>
			    											<a href="index.php?<?= rebuildQueryString(['orderby'=>'nom','sens'=>($_GET["orderby"] == 'nom' ? ($_GET["sens"] == 'desc' ? 'asc' : 'desc') : $_GET["sens"])])?>"> 
    														    <?= L('ProgrammeFidelite')?> <?= '<i class="fa fa-sort'.(($_GET["orderby"] == 'nom' ? ($_GET["sens"] == 'desc' ? '-desc' : '-asc') : '')).'"></i>'?>
														    </a>
														</th>
														<th>
														...
														<th>
			    											<a href="index.php?<?= rebuildQueryString(['orderby'=>'id_facture','sens'=>($_GET["orderby"] == 'id_facture' ? ($_GET["sens"] == 'desc' ? 'asc' : 'desc') : $_GET["sens"])])?>"> 
    														    <?= L("facture","o") ?> <?= '<i class="fa fa-sort'.(($_GET["orderby"] == 'id_facture' ? ($_GET["sens"] == 'desc' ? '-desc' : '-asc') : '')).'"></i>'?>
														    </a>
														</th>
													</tr>
												</thead>
