...
while ( $uneLignePoints = $resultPoints->fetch_assoc() ){
	...
	if ( !isset($arrayData[$ID_MAGASIN]) ){
	    //initialisation de l'entrée magasin (si non existant)
																...
																foreach ( $data['data'] as $jour => $rowRaport){
																	...
																	<tr>
																		<?php if(has_rights('rap_journalier')){?>
																			<td><a href="?p=rap_journalier&id=<?= $ID_MAGASIN?>&date=<?php echo $jour;?>"><?= formatDateUTF8($jour)?></a></td>
																		<?php }else{?>
																			<td><?= formatDateUTF8($jour)?></td>
																		<?php }?>
														...
														if(count($data['data']) > 0){
															...
															<tfoot>
																<tr>
																	...
																	<td class="text-right"><?= nfsnd($data['totalFactures'])?></td>
																	<td class="text-right"><?= nfs($data['totalEmis'])?></td>
																	<td class="text-right"><?= nfs($data['totalEmisBrut'])?></td>
																	<td class="text-right"><?= nfs($data['totalRecu'])?></td>
																	<td class="text-right"><?= nfs($data['totalRecu'])?></td>
																	<td class="text-right"><?= nfs($data['totalPointsBrut'])?></td>
																</tr>
															</tfoot>
															<?php
														}?>
