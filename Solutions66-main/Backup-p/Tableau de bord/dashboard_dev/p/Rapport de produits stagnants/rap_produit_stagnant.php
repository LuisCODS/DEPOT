...
if ( $_GET["search"] == "1" ){
	...
	while( $uneLigneAgence = $resultMag->fetch_assoc() ){
		...
		if( $_GET["showItemNeverSold"] != "1" ){
		    $joinSoldOnce = " INNER JOIN ( SELECT fac.id_article, fac.type FROM $nomDB.facture_item `fac` ) soldOnceItem ON article.id_article = soldOnceItem.id_article AND soldOnceItem.type = 'PLU' ";
		}
		
		// Articles à exclure
		$enonce = "SELECT facture_item.id_article 
		            FROM $nomDB.facture_item
		                INNER JOIN $nomDB.facture using (id_facture)
		...
		$enonce = "SELECT article.id_article, article.desc_fr, article.PLU, article.stock, laf.num_four  
		            FROM $nomDB.article
                	    INNER JOIN $nomDB.link_article_four `laf` using (id_article) 
                	    $joinSoldOnce 
                	WHERE  laf.discontinued IS NULL AND article.stock > 0 ". (($listFD)? "AND ".implode(" AND ", $listFD) :"") .
                	" GROUP BY article.id_article";

    	$resultArticle = query($enonce,[],$dbAnimoCaisse);
