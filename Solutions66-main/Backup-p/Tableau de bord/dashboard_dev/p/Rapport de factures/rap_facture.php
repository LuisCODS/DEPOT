...
foreach ( $listID_MAGASIN as $ID_MAGASIN ){
    $data[$ID_MAGASIN] = [ "lignes"=>[],   "nbrefact"=>0,  "totalsanstaxe"=>0, "totalavectaxe"=>0 ];
	...
	while( $uneLigneFact = $resultFactItem->fetch_assoc() ){
	    ...
	    //$uneLigneFact["paiement"]
	    $uneLigneFact["paiement"] = ["lignes"=>[],"montant"=>0];
...
$listTriPosible = ["date_insert","id_facture","grandtotal","soustotal"];
...
if ( $_GET["getFile"] == "1" and $data ){
	...
	if ( $_GET['from'] and $_GET['to'] ){
		...
		$listSoustitre[] = ["date", L("2 derniers jours") ];
	}

	$listEnteteColonne = [
			[ 
			    ["text"=>"Date","width"=>35,"align"=>"L"], ["text"=>L("#Facture",'o'),"width"=>25,"align"=>"C"],["text"=>L("Total (sans taxes)",'o'),"width"=>20,"align"=>"R"],["text"=>L("mode_paiement",'o'),"width"=>35,"align"=>"C"],
			    ["text"=>L("Notes",'o'),"width"=>25,"align"=>"R"],["text"=>L("carte Animo",'o'),"width"=>22,"align"=>"C"],["text"=>L("$ Animo",'o'),"width"=>18,"align"=>"R"],
			],
	];
    													...
    													<div class="fixed-table-body" style="min-height: 200px;">
    														...
    														<thead>
    															<tr>
    																<th></th>
    																<th><?= L("date") ?></th>
    																<th style="text-align:center"><?= L("no_facture","o") ?></th>
    																<th><?= L("mode_paiement","o") ?></th>
    																<th style="text-align:right"><?= L("totalsanstaxes") ?></th>
    																<th style="text-align:center"><?= L("notes") ?></th>
    																<th><?= L("carteanimo") ?></th>
    																<th style="text-align:right">$ Animo</th>
    															</tr>
    														</thead>
    														<tbody>
    															<?php
    															...
    															foreach ($dataMag["lignes"] as $rowRapport) {
    																...
    																<tr>
    																	<td><?= ++$count ?></td>
    																	<td><?= date("Y-m-d H:i:s",strtotime($rowRapport["date_insert"])) ?></td>
    																	<td style="text-align:center"><a href="javascript:;" class="ajaxModal" data-modal-url="ajax/modals/viewFacture.php?id_facture=<?= $rowRapport["id_facture"]?>&ID_MAGASIN=<?= $ID_MAGASIN?>"><?= $rowRapport["id_facture"]?></a></td>
    																	<td ><?php
    																	foreach($rowRapport["paiement"]["lignes"] as $unPaiement){
    																		echo $unPaiement["label"]."<br />";
    																	}
    																	?></td>
    																	<td style="text-align:right"><?= formatPrix($rowRapport["soustotal"])?></td>
    																	<td><?= $rowRapport["notes"]?></td>
    																	<td><a href="?p=list_clients&carte=<?= $rowRapport["num_avantages"]?>"><?= $rowRapport["num_avantages"]?></a></td>
