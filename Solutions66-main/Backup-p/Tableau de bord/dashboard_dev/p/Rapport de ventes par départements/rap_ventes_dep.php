...
$data = ["lignes"=>[],"nbfacture"=>0,"nbitem"=>0,"totalsanstaxe"=>0,"totalavectaxe"=>0,"nbes"=>0,"totales"=>0];
...
function getPourcentTaxeFromDate($date){
	...
	if ( preg_match('#^(\d{4,5})\-(\d{2})\-(\d{2})$#',$date,$matches) or preg_match('#^(\d{4,5})\-(\d{2})\-(\d{2}) \d{2}:\d{2}:\d{2}$#',$date,$matches) ){
		...
		if ( $uneLigneConfig = $result->fetch_assoc() ){
			$cache_getPourcentTaxeFromDate[$aa."-".$mm] = ["taxe1"=>$uneLigneConfig["taxe1"],"taxe2"=>$uneLigneConfig["taxe2"],"taxe3"=>$uneLigneConfig["taxe3"],"taxe4"=>$uneLigneConfig["taxe4"]];
			return $cache_getPourcentTaxeFromDate[$aa."-".$mm];
		}
	}
...
while( $uneLigneFact = $resulFact->fetch_assoc() ){
	...
	while( $uneLigneFactItem = $resulFactItem->fetch_assoc() ){
		...
		if ( !isset($data["lignes"][$id_departement]) ){
			...
			$data["lignes"][$id_departement] = ["nbfacture"=>0,"nbitem"=>0,"totalsanstaxe"=>0,"totalavectaxe"=>0,"nbes"=>0,"totales"=>0,"nom"=>(($uneLigneDep["nom"])?:"Département #".$id_departement)];
		}
...
foreach ( $data["lignes"] as $id_departement => $infoDep ){
	...
	$data["totalavectaxe"] += round($infoDep["totalavectaxe"],2);
}


$listTriPosible = ["nom","totalavectaxe","totalsanstaxe","nbitem","totales","nbes","nbfacture"];
...
uasort( $data["lignes"], function($a,$b){
	if ( strtoupper($a[$_GET["orderby"]]) < strtoupper($b[$_GET["orderby"]]) ){
		return ($_GET["sens"]=="desc")?1:-1;
	} elseif( strtoupper($a[$_GET["orderby"]]) > strtoupper($b[$_GET["orderby"]]) ){
		return ($_GET["sens"]=="desc")?-1:1;
	}
	return 0;
});
													...
													<thead>
														<tr>
															...
															<th style="text-align:center;">
																<?php echo L("nb. article","o");?>
															</th>
															<th style="text-align:center;">
																Nb. escomptes
															</th>
															<th style="text-align:right;">
																Total avant escompte
															</th>
															<th style="text-align:right;">
																Total des escomptes
															</th>
													...
													<tbody>
														...
														foreach ($data["lignes"] as $id_departement => $rowRaport) {
															...
															<tr>
																...
																<td style="text-align:center;">
																	<?= ($rowRaport["nbes"])?$rowRaport["nbes"]:"N/A" ?>
																</td>
																<td style="text-align:right;">
																	<a href="index.php?<?= rebuildQueryString(["p"=>"rap_ventes_dep_details","id_departement"=>$id_departement],["orderby",]) ?>"><?= formatPrix($rowRaport['totalsanstaxe']); ?></a>
																</td>
																<td style="text-align:right;">
																	<?= ($rowRaport["totales"])?formatPrix($rowRaport["totales"]):"N/A"?>
																</td>
																<td style="text-align:right;">
																	<?= formatPrix($rowRaport['totalsanstaxe'] + $rowRaport["totales"]); ?>
																</td>
															</tr>
															<?php
														} ?>
