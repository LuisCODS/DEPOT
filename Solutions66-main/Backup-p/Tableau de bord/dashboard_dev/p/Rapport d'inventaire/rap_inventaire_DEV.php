	...
	foreach ( $listID_MAGASIN as $ID_MAGASIN ){
		...
		while ($uneLigneArticle = $resultArt->fetch_assoc()){
			...
			$enonce =  "select fournisseur.*, link_article_four.num_four, link_article_four.prix_coutant, link_article_four.prix_caisse, link_article_four.id_link_article_four
						...
						where link_article_four.id_article = ? AND link_article_four.prix_coutant != '' 
					order by discontinued=1 asc, link_article_four.date_update desc
						limit 1";
...
if ( $_GET["getFile"] == "1" and $data ){
	...
	$listEnteteColonne = [
		[ 
			["text"=>"ID","width"=>13,"align"=>"R"],
			["text"=>"Nom du produit","width"=>150,"align"=>"L"],
			["text"=>L("Distrubuteur(s)",'o'),"width"=>60,"align"=>"L"],
			["text"=>L("Qté",'o'),"width"=>11,"align"=>"C"],
	...
	foreach ( $data as $ID_MAGASIN => $dataMag ){
		...
		foreach ( $dataMag["liste"] as $uneLigneArticle){
			$listChamps = [];
			$listChamps[0] = $uneLigneArticle["id_article"];
			$listChamps[1] = $uneLigneArticle["desc_fr"]." (".implode("|",$uneLigneArticle["listPLU"]).")";
	
			if ( $uneLigneArticle["uneLigneFour"] ){
				if ( $uneLigneArticle["boite_nb"] > 1){
					$listChamps[2] = $uneLigneArticle["uneLigneFour"]["nom"] .' - '.$uneLigneArticle["uneLigneFour"]["num_four"].' - '.formatPrix($uneLigneArticle["uneLigneFour"]["prix_caisse"]).' ('.$uneLigneArticle["boite_nb"].')';
				}else{
					$listChamps[2] = $uneLigneArticle["uneLigneFour"]["nom"].' - '.$uneLigneArticle["uneLigneFour"]["num_four"].' - '.formatPrix($uneLigneArticle["uneLigneFour"]["prix_coutant"]);
				}
			}
	
			$listChamps[3] = $uneLigneArticle["stock"];
			$listChamps[4] = nfs($uneLigneArticle["totalCost"]);
	
			$rapport->writeLigneRapport3wrap( $listChamps );
		}
		
		$rapport->writeLigneGrandTotal( [ "", L("nombre de produit")." : ".sizeof($dataMag["liste"]),null, $dataMag["totalStock"], nfs($dataMag["totalCost"])], [false,false,false,true,true] );
	}
