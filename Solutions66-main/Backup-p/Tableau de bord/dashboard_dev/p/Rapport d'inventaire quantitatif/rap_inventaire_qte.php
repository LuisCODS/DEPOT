...
if ( $_GET["search"] == "1" ){
	...
	if($_POST['order']!=''){ $order = $_POST['order']; }else{ $order = $listColonneTri[0];}
	...
	foreach ( $listID_MAGASIN as $ID_MAGASIN ){
		...
		$data[$ID_MAGASIN] = ["liste_fournisseur"=>[], "totalCost"=>0, "totalStock"=>0];
...
if ( $_GET["getFile"] == "1" and $data ){
	...
	//$rapport->writeLigneGrandTotal( [ L("nombre de produit")." : ".sizeof($data["liste"]),null, $data["totalStock"], nfs($data["totalCost"])], [false,false,true,true] );
