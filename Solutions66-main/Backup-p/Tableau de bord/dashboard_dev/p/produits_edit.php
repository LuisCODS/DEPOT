...
La BD : animoetc_caisse_default
_____________________________________________________________________________________________________
Les tables: 
article,article_desc,article_categorie_link,prix_change,link_article_besoin,link_article_four,
article_photo,fournisseur,marques,departement, prix,tailles,couleurs,races,stades,recettes,besoins,types
*/
...
if(isset($_POST["edit_produit"])){    
	...
	try{
		...
		if ( sizeof($listPLU) > 0 ){
			...
			//SI  erreur sur la query ou que la query return un résultat 
		...
		// ARTICLE		
		$arrayArticle = [];
		$arrayArticle["id_departement"] = trim($_POST["id_departement"]);// required
		$arrayArticle["desc_fr"] = trim($_POST["nom_fr"]); // required
		$arrayArticle["desc_en"] = trim($_POST["nom_en"]);
		$arrayArticle["PLU"] = trim($_POST["PLU"]); // required
		...
		if ( $rowArticle ){
			...
			foreach( $arrayArticle as $champ=>$valeur ){
				//vex( [(string)$valeur, (string)$rowArticle[$champ], $champ ] );
		...
		// __________________AJOUT DANS TABLE PRIX ($arrayPrix): liée au article_________________________________
		...
		if ( $_POST["prix_change_prix"] != ""  && preg_match('#^\s*[0-9\s]{1,8}([.,][0-9]{0,2})?\s*?$#', $_POST["prix_change_prix"]) ){
			...
			$arrayPrix["id_article"] = $id_article;//La reference
			$arrayPrix["qte"] = "1";
			$arrayPrix["date_update"] = date("Y-m-d H:i:s");
			$arrayPrix["date_insert"] = date("Y-m-d H:i:s");
			$arrayPrix["id_staff"] = $_SESSION["utilisateur"]["id_utilisateur"];
		...
		//_______________________  AJOUT TABLE LINK_ARTICLE_BESOIN($arrayDB3): liée au article _______________________ 
		
		if(isset($_POST['id_besoins']) && !empty($_POST['id_besoins'])){
			$arrayDB3 = array();
			$arrayDB3["id_article"] = $id_article;//La reference 
			// Filtre les éléments envoyées grâce à la function (is_numeric) qui retourne true si value est un nombre ou une chaîne numérique, false sinon.
			$besoins = array_filter($_POST['id_besoins'],"is_numeric");
        ...
        // _______________________ TABLE article_desc($arrayDB): liée au article _______________________ 
		...
		//redirect("index.php?p=price_error", $DEBUG_DB);
												...
												*/ ?>
												<div class="col-4">
													...
													<div class="ui dynamic checkbox mb-1">
														<input type="checkbox" name="enVedette" value="1" <?= $rowDesc["enVedette"] == "1" ? "checked ":""?> >
                                    ...
                                    var onSubmitAddArticle = function(monForm){
                                        ...
                                        try{
                                            ...
                                            */
                                            ...
                                            //if( monForm.id_departement.value != "" && monForm.id_categorie.value != "" )
                                            if( monForm.id_departement.value != "" )
                                            {
